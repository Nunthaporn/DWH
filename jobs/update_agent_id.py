...
def extract_agent_mapping() -> pd.DataFrame:
    """
    ดึง agent จาก MySQL + quotation จาก Postgres แล้ว normalize ให้ได้ mapping:
      [quotation_num, agent_id] (ตัวสะกดมาตรฐานเท่าที่ทำได้ฝั่ง Python)
    หมายเหตุ: casing สุดท้ายจะ normalize อีกครั้งในฝั่ง Postgres ด้วยการ join dim_agent
    """
    ...
    # right join → คง quotation ทั้งหมด
    df_m1 = pd.merge(df_career, df_fact, on="quotation_num", how="right")

    # dim_agent for standardization group-by base_id (prefer defect ถ้ามี)
    with target_engine.begin() as tconn:
    ...
    # เลือก agent_id_final จาก main ถ้ามี ไม่งั้นใช้ของเดิม
    ...
    df_join["agent_id_final"] = np.where(df_join["agent_id_main"].notna(),
                                         df_join["agent_id_main"], df_join["agent_id_m1"])
    ...
    # ตัดค่าว่างออก (จะไม่ upsert แถวที่ไม่มี agent)
    df_out = df_out.dropna(subset=["quotation_num"])
    print(f"✅ extract_agent_mapping → rows: {len(df_out):,}")
    return df_out
...
def stage_dim_agent_temp(df_map: pd.DataFrame) -> str:
    ...
    และ normalize casing ให้ตรงกับ dim_agent ใน DB
    """
    tbl = "dim_agent_temp"
    ...
    tmp = df_map.copy()
    tmp = tmp.replace(["None", "none", "nan", "NaN", "NaT", ""], pd.NA)
    tmp["quotation_num"] = normalize_str_col(tmp["quotation_num"])
    tmp["agent_id"] = normalize_str_col(tmp["agent_id"])
    tmp = tmp.dropna(subset=["quotation_num"])  # key ต้องมี

    with target_engine.begin() as conn:
        tmp.to_sql(tbl, con=conn, schema=PG_SCHEMA, if_exists="replace", index=False, method="multi")
    ...
    print(f"✅ staged {len(tmp):,} rows → {full_tbl}")
    ...
    return full_tbl  # ส่งคืนชื่อ table แบบ fully-qualified

@op
def update_fact_from_temp(temp_table_name: str) -> int:
    """
    อัปเดต {schema}.fact_sales_quotation.agent_id จาก temp โดย join {schema}.dim_agent
    (กัน FK violation และอัปเดตเฉพาะแถวที่ค่าต่างจริง)
    """
